...
class ComplianceScanner:
    ...
    async def quick_scan(self) -> ScanResult:
        ...
        try:
            ...
            result.end_time = datetime.now()
            result.findings = [{"error": f"Scan failed: {str(e)}"}]
            return result
    ...
    def filter_scans_by_type(self, scan_type: ScanType) -> List[ScanResult]:
        ...
        for scan in self.scan_history:
            if hasattr(scan, 'scan_type') and scan.scan_type == scan_type:
                filtered_scans.append(scan)
            elif isinstance(scan, dict) and scan.get('scan_type') == scan_type.value:
                filtered_scans.append(scan)
